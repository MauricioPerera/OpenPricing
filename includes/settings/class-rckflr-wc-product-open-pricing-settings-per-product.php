...
class Alg_WC_Product_Open_Pricing_Settings_Per_Product {
	...
	function admin_notices() {
		...
		echo '<div class="error"><p><div class="message">' .
			sprintf( 'Free plugin\'s version is limited to only one open pricing product enabled at a time. You will need to get <a href="%s" target="_blank">Product Open Pricing for WooCommerce Pro</a> to add unlimited number of open pricing products.', 'https://rckflr.com/products/open-pricing/' ) .
		'</div></p></div>';
	}
	...
	function get_meta_box_options() {
		$options = array(
			array(
				...
				'type'       => 'select',
				'options'    => array(
					'yes' => __( 'Yes', 'open-pricing' ),
					'no'  => __( 'No', 'open-pricing' ),
				),
				'title'      => '<strong>' . __( 'Enabled', 'open-pricing' ) . '</strong>',
			),
			array(
				'name'       => 'alg_wc_product_open_pricing_default_price',
				'default'    => '',
				'type'       => 'price',
				'title'      => __( 'Default price', 'open-pricing' ) . ' (' . get_woocommerce_currency_symbol() . ')',
				'tooltip'    => __( 'Default (i.e. Suggested) price', 'open-pricing' ),
			),
			array(
				'name'       => 'alg_wc_product_open_pricing_min_price',
				'default'    => 1,
				'type'       => 'price',
				'title'      => __( 'Min price', 'open-pricing' ) . ' (' . get_woocommerce_currency_symbol() . ')',
			),
			array(
				'name'       => 'alg_wc_product_open_pricing_max_price',
				'default'    => '',
				'type'       => 'price',
				'title'      => __( 'Max price', 'open-pricing' ) . ' (' . get_woocommerce_currency_symbol() . ')',
			),
		);
		return $options;
	}
	...
	function add_meta_box() {
		add_meta_box(
			...
			__( 'Product Open Pricing', 'open-pricing' ),
